namespace IntegrationV2.Files.cs.Domains.MeetingDomain.Synchronization {
	...
	public class CalendarSyncSession : IBackgroundTask<SyncSessionArguments>, IUserConnectionRequired, ISyncSession {
		...
		/// <summary>
		/// <see cref="IFeatureUtilities"/> implementation instance.
		/// </summary>
		private IFeatureUtilities _featureUtilities;
		...
		private void SetDependencies(UserConnection uc, SyncSessionArguments runOptions) {
			...
			_participantRepository = _participantRepository ?? ClassFactory.Get<IParticipantRepository>(new ConstructorArgument("uc", _uc),
				new ConstructorArgument("sessionId", _log.SessionId),
				new ConstructorArgument("calendarRepository", _calendarRepository));
			_featureUtilities = _featureUtilities ?? ClassFactory.Get<IFeatureUtilities>();
		}
		...
		private bool TrySyncImportedMeetings(Calendar calendar, List<MeetingDto> rawData, out List<Meeting> allMeetings) {
			var isLoadedAllImportItems = true;
			allMeetings = new List<Meeting>();
			var invitesSentMeetings = new List<Meeting>();
			foreach (var rawMeeting in rawData.OrderByDescending(rd => rd.InvitesSent)) {
				...
				try {
					...
					if (itemLocked) {
						...
						allMeetings.AddRangeIfNotExists(meetings);
						invitesSentMeetings.AddRangeIfNotExists(meetings.Where(m => m.InvitesSent));
						...
						if (_featureUtilities.GetIsFeatureEnabled(_uc, "CreateCurrentUserParticipant")) {
		...
		private void DeleteMeetingsDifference(Calendar calendar, ICalendarClient calendarClient, List<MeetingDto> rawData,
			...
			if (!_featureUtilities.GetIsFeatureEnabled(_uc, "InternalMeetingsDelete")) {
				return;
			}
		...
		private void DeleteExternalMeetingsDuplicate(List<Meeting> meetings) {
		...
		private List<Meeting> GetMeetingsForImport(List<MeetingDto> rawData, Calendar calendar) {
			...
			foreach (var rawMeeting in rawData) {
				result.AddRangeIfNotExists(GetMeetingForImport(rawMeeting, calendar));
			}
			return result;
		}
		...
		private List<Meeting> GetImportedMeetingsForDelete(List<MeetingDto> rawData, Calendar calendar, ICalendarClient calendarClient,
				DateTime syncStartDate) {
			bool meetingCondition(Meeting m) => m.Calendar?.Id == calendar.Id && m.State != SyncState.Deleted && m.CanBeExported();
		...
		private bool IsMeetingMissingInCalendar(Calendar calendar, MeetingDto meeting) {
			...
			return meeting.IsCancelled
				...
				|| (isOrganizer && !_featureUtilities.GetIsFeatureEnabled(_uc, "NotDeleteDesktopMeetings"));
		}
		...
		private List<Meeting> FilterLocalCopies(List<Meeting> meetings) {
			...
			return copiesToDelete.Distinct(ComparerUtils.GetComparer<Meeting>()).ToList();
		}
		...
		private void FillMeeting(Meeting meeting, MeetingDto meetingDto) {
			...
			foreach (var participant in participants) {
				if (meetingDto.Participants.ContainsKey(participant.EmailAddress)) {
					...
					if (participant.ContactId == meeting.Calendar.OwnerId) {
